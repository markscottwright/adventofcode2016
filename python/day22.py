...
class Node:
    def __init__(self, name, size, used):
        self.name = name
        self.size = size
        self.used = used
        m = re.match("/dev/grid/node-x([0-9]+)-y([0-9]+)", self.name)
        self.x = int(m.group(1))
        self.y = int(m.group(2))

    def __repr__(self):
        return f"Node({self.name}, {self.size}, {self.used})"

    def __str__(self):
        return f"Node {self.x},{self.y} size={self.size} used={self.used}"

    def fits_in(self, o):
        return self.used <= (o.size - o.used)
...
def parse(l):
    m = re.match(r"^(/\S+)\s+([0-9]+)T\s+([0-9]+)T\s+([0-9]+)T", l)
    if m:
        n = Node(m.group(1), int(m.group(2)), int(m.group(3)))
        assert int(m.group(4)) == n.size-n.used
        return n
    else:
        return None
...
def viable_pairs(nodes):
    for a in nodes:
        for b in nodes:
            if a != b and not a.is_empty() and a.fits_in(b):
                yield [a,b]
...
def move_to_origin(nodes, data_node):
    """
    """
    pass


nodes = [parse(l) for l in INPUT.split('\n')]
nodes = [n for n in nodes if n is not None]
print("Part 1:", sum(1 for p in viable_pairs(nodes)))
print("max x", highest_x(nodes))
